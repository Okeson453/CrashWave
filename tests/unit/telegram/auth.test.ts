...
describe('Telegram Auth Middleware', () => {
  ...
  describe('allowlist blocking', () => {
    it('allows authorized user in private chat', async () => {
    ...
    it('rejects unauthorized user', async () => {
      const ctx = createMockContext({
        from: { id: 999999999, is_bot: false, username: 'hacker', first_name: 'Bad', last_name: 'Actor' },
      });
      const next = jest.fn().mockResolvedValue(undefined);
    ...
    it('rejects user not in allowlist with correct error message', async () => {
      const ctx = createMockContext({
        from: { id: 111111111, is_bot: false, username: 'unauthorized', first_name: 'Un', last_name: 'Authorized' },
      });
      const next = jest.fn().mockResolvedValue(undefined);
...
describe('getOperatorIdentity', () => {
  it('extracts operator identity correctly', () => {
    ...
    expect(identity.lastName).toBe('User');
  });
