/**
 * Early Crash Simulation Scenario
 * Simulates rounds that crash immediately at or near 1.00x.
 */
...
describe('Simulation: Early Crash', () => {
  ...
  it('should detect early crash rounds', async () => {
    ...
    // Simulate early crash round
    adapter.emitEvent({
      type: 'round-started', roundId: 'early-001', multiplier: 1.0,
      crashPoint: null, phase: 'running', source: 'dom',
      confidence: 'high', timestamp: new Date().toISOString(), latencyMs: 50,
    });
    adapter.emitEvent({
      type: 'multiplier-tick', roundId: 'early-001', multiplier: 1.01,
      crashPoint: null, phase: 'running', source: 'dom',
      confidence: 'high', timestamp: new Date().toISOString(), latencyMs: 50,
    });
    adapter.emitEvent({
      type: 'round-crashed', roundId: 'early-001', multiplier: 1.01,
      crashPoint: 1.01, phase: 'crashed', source: 'dom',
      confidence: 'high', timestamp: new Date().toISOString(), latencyMs: 50,
    });

    expect(crashes.length).toBeGreaterThanOrEqual(1);
    expect(crashes[0].crashPoint).toBeGreaterThanOrEqual(1.0);
    expect(crashes[0].crashPoint).toBeLessThanOrEqual(1.1);
  });
  ...
  it('should handle rapid round transitions', async () => {
    ...
    adapter.emitEvent({
      type: 'round-crashed', roundId: 'early-001', multiplier: 1.02,
      crashPoint: 1.02, phase: 'crashed', source: 'dom',
      confidence: 'high', timestamp: new Date().toISOString(), latencyMs: 50,
    });

    // Round 2
    adapter.emitEvent({
      type: 'round-started', roundId: 'early-002', multiplier: 1.0,
      crashPoint: null, phase: 'running', source: 'dom',
      confidence: 'high', timestamp: new Date().toISOString(), latencyMs: 50,
    });
    adapter.emitEvent({
      type: 'round-crashed', roundId: 'early-002', multiplier: 1.05,
      crashPoint: 1.05, phase: 'crashed', source: 'dom',
      confidence: 'high', timestamp: new Date().toISOString(), latencyMs: 50,
    });

    expect(observer.getRoundCount()).toBeGreaterThanOrEqual(2);
  });
});
