 ...
 * Transaction helpers.
 *
 * Personal-use adaptation: the advanced Crash build has a multi-tenant
 * `getTenantId()` from `../platform/tenant-context` (which was removed
 * per spec §2.2 in personal use). In personal use, there is no
 * multi-tenancy, so `getTenantId()` always returns null. The
 * transaction wrapper skips the `set_config('app.tenant_id', ...)` call
...
/** Personal-use stub: always returns null (no multi-tenancy). */
