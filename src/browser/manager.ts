...
import { stealthLaunchArgs, applyStealthToContext, applyStealthToPage, STEALTH_BROWSER_ARGS } from './stealth';
...
export class BrowserManager {
  ...
  async launch(): Promise<Page> {
    ...
    try {
      ...
      // Never allow --enable-automation to leak in
      const args = [...new Set([...stealthArgs, ...(this.options.args ?? []), ...STEALTH_BROWSER_ARGS])].filter(
        (a) => !String(a).includes('enable-automation')
      );

      // Resolve proxy (from launch options or ProxyManager-compatible config on options)
      ...
      // Apply advanced stealth (context init script + page CDP)
      ...
      throw new CriticalError(`Browser launch failed: ${message}`, 'BROWSER_LAUNCH_FAILED');
    }
  }
  ...
  async navigate(url: string, waitUntil: 'load' | 'domcontentloaded' | 'networkidle' = 'networkidle'): Promise<NavigationResult> {
    ...
    try {
      await withTimeout(
        this.page.goto(url, { waitUntil, timeout: this.options.timeoutMs }),
      ...
      this.logger.info(
        { component: 'BrowserManager', url: currentUrl, loadTimeMs },
        'Navigation complete'
      );
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error({ component: 'BrowserManager', url, error: message }, 'Navigation failed');
      return {
        success: false,
        url: this.page.url(),
        title: '',
  ...
  async forceKill(): Promise<void> {
    ...
    try {
      if (this.browser) {
        // Playwright doesn't expose PID directly, but we can try to close
        await this.browser.close();
      }
    } catch {
      // Ignore
